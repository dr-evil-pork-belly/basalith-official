...
export default function ContributorsPage() {
  ...
  // Derive item counts from archive-items
  useEffect(() => {
    try {
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-end justify-between mb-8 gap-4">
        ...
        <button
          ...
          className="btn-monolith-amber shrink-0 !py-2.5 !px-5 !text-[0.7rem]"
        >
      ...
      {showForm && (
        <form
          onSubmit={handleAdd}
          className="rounded-sm border px-7 py-7 mb-8"
          style={{ background: '#111112', borderColor: 'rgba(196,162,74,0.15)' }}
        >
          <p className="font-sans text-[0.62rem] tracking-[0.14em] uppercase mb-6" style={{ color: 'rgba(196,162,74,0.7)' }}>
            New Contributor
          </p>
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: '#111112' }}>
                {['Name', 'Role', 'Labels', 'Added', ''].map(h => (
                  <th key={h} className="font-sans text-[0.56rem] tracking-[0.12em] uppercase text-left px-5 py-3" style={{ color: '#3A3F44', fontWeight: 600 }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {contributors.map((c, i) => (
                <tr
                  key={c.id}
                  style={{
                    borderBottom: i < contributors.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
                    background: '#111112',
                  }}
                >
                  ...
                  <td className="px-5 py-4">
                    <span
                      className="font-sans text-[0.6rem] tracking-[0.08em] uppercase px-2 py-1 rounded-sm"
                  ...
                  <td className="px-5 py-4 text-right">
                    <button
                      onClick={() => remove(c.id)}
                      className="font-sans text-[0.6rem] tracking-[0.08em] uppercase transition-colors duration-200"
                      style={{ color: '#3A3F44' }}
                    >
                      Remove
                    </button>
