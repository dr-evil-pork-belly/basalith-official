...
<body style="background:#0A0908;font-family:Georgia,serif;color:#F0EDE6;max-width:600px;margin:0 auto;padding:32px">
  ...
  <p style="font-family:'Courier New',monospace;font-size:11px;letter-spacing:2px;color:#8B9196;margin:0 0 28px">VOICE PORTRAIT · ${monthLabel.toUpperCase()}</p>
  ...
  <div style="border-left:3px solid rgba(196,162,74,0.2);padding:20px 24px;margin:0 0 32px;background:rgba(196,162,74,0.03)">
    <p style="font-family:'Courier New',monospace;font-size:11px;letter-spacing:2px;color:#8B9196;margin:0 0 12px">TRANSCRIPT</p>
  ...
  <p style="font-family:Georgia,serif;font-size:13px;font-style:italic;color:#8B9196;line-height:1.7;margin:0 0 24px">
    This was generated by the entity based on what it has learned.
    The voice is the owner's voice.
  ...
  for (const archive of archives ?? []) {
    try {
      ...
      const scriptRes = await anthropic.messages.create({
        ...
        max_tokens: 300,
        system:     buildScriptSystemPrompt(archive.owner_name ?? 'the owner', archive.name, lang),
        messages: [{
          role:    'user',
