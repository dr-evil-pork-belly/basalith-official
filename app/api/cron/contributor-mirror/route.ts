...
export async function GET(req: NextRequest) {
  ...
  for (const archive of archives ?? []) {
    try {
      ...
      if (!recentAnswers?.length) { skipped.push(`${archive.name} (no answers)`); continue }

      // Get contributor names
      const contributorIds = [...new Set(recentAnswers.map(a => a.contributor_id).filter(Boolean))]
      ...
      const answers = recentAnswers.map(a => ({
        ...
        answer:          (a.answer_text || '').substring(0, 150) + ((a.answer_text?.length ?? 0) > 150 ? '…' : ''),
      }))

      const lang      = archive.preferred_language ?? 'en'
      const firstName = archive.owner_name?.split(' ')[0] ?? 'there'
      const subject   = lang === 'zh' ? `本月您的家人分享的内容 · ${archive.name}`
      ...
      await resend.emails.send({
        ...
        headers: {
          ...
          'X-Entity-Ref-ID':  `basalith-mirror-${archive.id}-${calMonth}`,
          'Precedence':       'bulk',
        },
      })
      ...
      await supabaseAdmin.from('owner_notifications').insert({
        ...
        type:       'contributor_mirror',
        subject,
        sent_to:    archive.owner_email,
        sent_at:    new Date().toISOString(),
        metadata:   { calMonth, answerCount: answers.length },
      })

      sent++
    } catch (err: any) {
      console.error(`[contributor-mirror] ${archive.id}:`, err.message)
