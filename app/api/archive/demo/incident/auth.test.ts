/**
 * Acceptance gate: item 4: the demo capture buffer endpoints are owner-gated.
 *
 ...
 * so what is under test is the guard itself: rejected without an owner session,
...
import { POST as startPOST } from '@/app/api/archive/demo/incident/start/route'
import { POST as turnPOST } from '@/app/api/archive/demo/incident/turn/route'
...
describe('demo incident buffer, owner auth guard', () => {
  it('start: rejects with 401 when there is no owner session', async () => {
    mockedGetSessionUser.mockResolvedValue(null)
    const res = await startPOST(post('http://localhost/api/archive/demo/incident/start', {}))
  ...
  it('start: accepts with 200 when an owner session is present', async () => {
    mockedGetSessionUser.mockResolvedValue({ userId: 'u1', email: 'owner@x.co', role: 'owner', archiveId: 'a1' })
    const res = await startPOST(post('http://localhost/api/archive/demo/incident/start', {}))
    const body = await res.json()
  ...
  it('turn: rejects with 401 when there is no owner session', async () => {
    mockedGetSessionUser.mockResolvedValue(null)
    const res = await turnPOST(post('http://localhost/api/archive/demo/incident/turn', { session: VALID_DEMO_SESSION, answer: 'A real answer here.' }))
    const body = await res.json()
  ...
  it('turn: accepts with 200 when an owner session is present', async () => {
    mockedGetSessionUser.mockResolvedValue({ userId: 'u1', email: 'owner@x.co', role: 'owner', archiveId: 'a1' })
    const res = await turnPOST(post('http://localhost/api/archive/demo/incident/turn', { session: VALID_DEMO_SESSION, answer: 'A real answer here.' }))
    const body = await res.json()
  ...
  it('turn: rejects a session that points at a real (non-demo) archive id', async () => {
    ...
    const res = await turnPOST(post('http://localhost/api/archive/demo/incident/turn', { session: realArchive, answer: 'A real answer here.' }))
    const body = await res.json()
