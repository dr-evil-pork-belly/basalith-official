...
function milestoneStage(depositCount: number): string {
  if (depositCount < 10)  return `Building the record (${depositCount} memories so far)`
...
export async function POST(req: Request) {
  try {
    ...
    const archiveName  = (archiveResult.data?.name ?? context?.archiveName ?? 'this Basalith') as string
    const ownerName    = (archiveResult.data?.owner_name ?? context?.ownerName ?? 'the owner') as string
...
You can save meaningful things the user shares to the record using the save_deposit tool. When someone shares a real memory, routine, value, or piece of knowledge, you may offer to save it. Only confirm something is saved AFTER the save_deposit tool has actually run. Never claim to have saved something you did not save through the tool.`
    ...
    if (!content) {
      ...
      if (error || !deposit) {
        ...
        toolResultText = `Saved to the record. Deposit id: ${savedDepositId}. Confirm to the user warmly and briefly that it is now on the record.`
      }
    }
