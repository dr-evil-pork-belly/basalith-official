...
export default function ArchivistDashboard() {
  ...
  async function fetchFromDB() {
    try {
      const res = await fetch('/api/archivist/dashboard')
  ...
  // Sprint progress toward next tier
      ...
      {!loading && archivist && (
        <div className="rounded-sm border border-border-subtle px-6 py-5 mb-8" style={{ background: '#111112' }}>
          <div className="flex items-baseline justify-between mb-3">
            <p className="font-sans text-[0.6rem] font-bold tracking-[0.18em] uppercase text-text-muted">
              Sprint Progress · This Month
            </p>
            <p className="font-sans text-[0.68rem]" style={{ color: '#C4A24A' }}>
              Next bonus: {nextSprint.bonus} at {nextSprint.threshold} closings
            </p>
          </div>
          <div className="h-1.5 rounded-full mb-2" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <div
              className="h-1.5 rounded-full transition-all duration-700"
              style={{ width: `${sprintPct}%`, background: 'rgba(196,162,74,0.8)' }}
            />
          </div>
          <p className="font-sans text-[0.65rem]" style={{ color: '#5C6166' }}>
            {sprintCount} / {nextSprint.threshold} closings this month
          </p>
        </div>
      )}
          ...
          {loading ? (
            ...
            <div className="px-6 py-10 text-center">
              <p className="font-serif italic text-text-muted" style={{ fontSize: '0.95rem' }}>
                No pending actions. Add prospects to your pipeline.
              </p>
            </div>
          ) : (
            <ul>
              {actions.map(a => (
                <li key={a.id} className="px-6 py-3 flex items-start justify-between gap-4 border-b border-border-subtle last:border-0">
                  ...
                  <span
                    className="font-sans text-[0.62rem] font-medium tracking-[0.06em] uppercase shrink-0 mt-0.5"
                    style={{ color: STATUS_COLOR[a.status] ?? '#5C6166' }}
                  >
                    {a.status}
                  </span>
                </li>
        ...
        <div className="rounded-sm border border-border-subtle p-6" style={{ background: '#111112' }}>
            ...
            <div className="flex flex-col gap-3">
              {pipelineStatuses.map(status => (
                <div key={status} className="flex items-center justify-between">
                  <span className="font-sans text-[0.72rem]" style={{ color: STATUS_COLOR[status] ?? '#5C6166' }}>
                    {status}
                  </span>
                  <span className="font-serif font-semibold text-text-primary" style={{ fontSize: '1.1rem', letterSpacing: '-0.02em' }}>
                    {pipeline[status] ?? 0}
                  </span>
                </div>
              ))}
          ...
          <div className="mt-5 pt-4 border-t border-border-subtle">
            <Link
              ...
              className="font-sans text-[0.68rem] font-medium no-underline transition-colors duration-150"
              style={{ color: '#C4A24A' }}
            >
              Manage Pipeline →
            </Link>
      ...
      {!loading && leaderboard.length > 0 && (
        <div className="rounded-sm border border-border-subtle mb-8" style={{ background: '#111112' }}>
          <div className="px-6 py-4 border-b border-border-subtle flex items-center justify-between">
            ...
            <Link href="/archivist/leaderboard" className="font-sans text-[0.62rem] no-underline" style={{ color: '#5C6166' }}>
              View all →
            </Link>
          </div>
          <table className="w-full">
            <tbody>
              {leaderboard.slice(0, 5).map((row, i) => (
                <tr
                  key={row.id}
                  style={i < Math.min(leaderboard.length, 5) - 1 ? { borderBottom: '1px solid rgba(255,255,255,0.04)' } : {}}
                >
                  <td className="px-5 py-3 w-8">
                    <span
                      className="font-serif font-semibold"
                      style={{ fontSize: '1rem', color: i === 0 ? '#C4A24A' : '#3A3F44', letterSpacing: '-0.02em' }}
                    >
                      {i + 1}
                    </span>
                  </td>
                  <td className="px-2 py-3">
                    <p
                      className="font-sans text-[0.8rem] font-medium"
                      style={{ color: row.id === archivist?.id ? '#C4A24A' : '#F0F0EE' }}
                    >
      ...
      {!loading && commissions.length > 0 && (
        <div className="rounded-sm border border-border-subtle mb-8" style={{ background: '#111112' }}>
          <div className="px-6 py-4 border-b border-border-subtle flex items-center justify-between">
            ...
            <Link href="/archivist/earnings" className="font-sans text-[0.62rem] no-underline" style={{ color: '#5C6166' }}>
              View all →
            </Link>
          </div>
          <ul>
            {commissions.slice(0, 5).map((c, i) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="font-sans text-[0.62rem] mt-0.5 capitalize" style={{ color: c.status === 'paid' ? '#4CAF50' : '#9DA3A8' }}>
                    {c.status}
                  </p>
                </div>
                <p
                  className="font-serif font-semibold shrink-0"
                  style={{ fontSize: '1rem', letterSpacing: '-0.02em', color: '#C4A24A' }}
                >
                  {fmt(c.amount_cents)}
                </p>
              </li>
