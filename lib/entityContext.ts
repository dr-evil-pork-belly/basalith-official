...
// The model used for live entity-chat generation. The fidelity eval (Tests A/B)
// must generate through the exact same model + builder as production, or the
// eval measures a different system than the one families talk to.
export const ENTITY_MODEL      = 'claude-opus-4-6'
export const ENTITY_MAX_TOKENS = 600
...
export async function buildEntitySystemPrompt(
  archiveId: string,
  ...
  // excludeDepositIds is used only by the eval runner to keep held-out
  // deposits out of retrieval (Test B measures fidelity, not recall).
  // Production calls always pass an empty array, so this is a no-op for them.
  if (excludeDepositIds.length > 0) {
    depositsQuery = depositsQuery.not('id', 'in', `(${excludeDepositIds.join(',')})`)
  }
...
/**
 * Generates the entity's answer to a prompt via the exact production path:
 * buildEntitySystemPrompt + the same model/token budget as entity-chat.
 * Used by the fidelity eval (Tests A/B) so eval generations are faithful to
...
export async function generateEntityResponse(
  ...
  const { systemPrompt, usedDepositIds } = await buildEntitySystemPrompt(archiveId, prompt, excludeDepositIds)
  ...
  const response = aiResponse.content[0].type === 'text' ? aiResponse.content[0].text : ''
  return { response, usedDepositIds }
}
